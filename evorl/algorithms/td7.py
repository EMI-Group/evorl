...
from evorl.utils.jax_utils import scan_and_mean, tree_stop_gradient, tree_get, right_shift_with_padding
...
class TD7Encoder(nn.Module):
    ...
    def zs(self, obs: jax.Array) -> jax.Array:
        z = obs
        for i, hdim in enumerate(self.f_layer_sizes):
            z = nn.Dense(hdim, name=f"zs_{i+1}")(z)
            z = nn.elu(z)
        z = nn.Dense(self.z_s_dim, name="zs_out")(z)
        return avg_l1_norm(z)

    @nn.compact
    def zsa(self, z_s: jax.Array, action: jax.Array) -> jax.Array:
        z = jnp.concatenate([z_s, action], axis=-1)
        for i, hdim in enumerate(self.g_layer_sizes):
            z = nn.Dense(hdim, name=f"zsa_{i+1}")(z)
            z = nn.elu(z)
        z = nn.Dense(self.z_sa_dim, name="zsa_out")(z)
        return z
...
class TD7Actor(nn.Module):
    ...
    def __call__(self, obs: jax.Array, z_s: jax.Array) -> jax.Array:
        a = nn.Dense(self.hidden_layer_sizes[0], name="l0")(obs)
        a = avg_l1_norm(a)
        a = jnp.concatenate([a, z_s], axis=-1)
        
        for i, hdim in enumerate(self.hidden_layer_sizes):
            a = nn.Dense(hdim, name=f"l{i+1}")(a)
            a = nn.relu(a)
            
        a = nn.Dense(self.action_size, name="l_out")(a)
        return nn.tanh(a)
...
class TD7Critic(nn.Module):
    ...
    def __call__(
        ...
        embeddings = jnp.concatenate([z_sa, z_s], axis=-1)

        # q1 network
        q1 = nn.Dense(self.hidden_layer_sizes[0], name="q1_0")(sa)
        q1 = avg_l1_norm(q1)
        q1 = jnp.concatenate([q1, embeddings], axis=-1)
        for i, hdim in enumerate(self.hidden_layer_sizes):
            q1 = nn.Dense(hdim, name=f"q1_{i+1}")(q1)
            q1 = nn.elu(q1)
        q1 = nn.Dense(1, name="q1_out")(q1)

        # q2 network
        q2 = nn.Dense(self.hidden_layer_sizes[0], name="q2_0")(sa)
        q2 = avg_l1_norm(q2)
        q2 = jnp.concatenate([q2, embeddings], axis=-1)
        for i, hdim in enumerate(self.hidden_layer_sizes):
            q2 = nn.Dense(hdim, name=f"q2_{i+1}")(q2)
            q2 = nn.elu(q2)
...
class TD7Agent(Agent):
    ...
    def init(
        ...
        dummy_z_s, dummy_z_sa = self.encoder_network.apply(encoder_params, dummy_obs, dummy_action)
        ...
        extra_state = PyTreeDict(
            max_q=jnp.array(-1e8, dtype=jnp.float32),
            min_q=jnp.array(1e8, dtype=jnp.float32),
            max_target=jnp.array(0.0, dtype=jnp.float32),
            min_target=jnp.array(0.0, dtype=jnp.float32),
            best_perf=jnp.array(-1e8, dtype=jnp.float32),
        )
    ...
    def evaluate_actions(
        ...
        actions = self.actor_network.apply(agent_state.params.checkpoint_actor_params, obs, z_s)
    ...
    def critic_loss(
        ...
        priority = jnp.maximum(td_error.max(axis=-1), self.min_priority) ** self.alpha
    ...
    def actor_loss(
        ...
        actions = self.actor_network.apply(agent_state.params.actor_params, obs, fixed_z_s)
...
def make_td7_agent(
    ...
    encoder_network = TD7Encoder(
        z_s_dim=z_s_dim, z_sa_dim=z_sa_dim, 
        f_layer_sizes=f_layer_sizes, g_layer_sizes=g_layer_sizes, 
        action_size=action_size
    )
    critic_network = TD7Critic(z_s_dim=z_s_dim, z_sa_dim=z_sa_dim, hidden_layer_sizes=critic_hidden_layer_sizes)
    actor_network = TD7Actor(z_s_dim=z_s_dim, hidden_layer_sizes=actor_hidden_layer_sizes, action_size=action_size)
...
class TD7Workflow(OffPolicyWorkflowTemplate):
    ...
    def _build_from_config(cls, config: DictConfig):
        assert config.rollout_episodes % config.num_envs == 0, "rollout_episodes must be divisible by num_envs"
    ...
    def step(self, state: State) -> tuple[MetricBase, State]:
        ...
        def _get_encoder_grad(params, agent_state, sample_batch, key):
        ...
        def _get_critic_grad(params, agent_state, sample_batch, key):
            temp_agent_state = agent_state.replace(
        ...
        def _get_actor_grad(params, agent_state, sample_batch, key):
            temp_agent_state = agent_state.replace(
        ...
        def _sample_and_update_fn(carry, t):
            ...
            (enc_loss, enc_loss_dict), enc_params, enc_opt_state = encoder_update_fn(
                enc_opt_state, agent_state.params.encoder_params, agent_state, sample_batch, enc_key
            )
            agent_state = agent_state.replace(
            ...
            # 2. Update Critic
            (critic_loss, critic_loss_dict), critic_params, critic_opt_state = critic_update_fn(
                critic_opt_state, agent_state.params.critic_params, agent_state, sample_batch, critic_key
            )
            agent_state = agent_state.replace(
            ...
            new_max = jnp.maximum(agent_state.extra_state.max_q, critic_loss_dict.batch_q_max)
            new_min = jnp.minimum(agent_state.extra_state.min_q, critic_loss_dict.batch_q_min)
            agent_state = agent_state.replace(
            ...
            def _update_actor(carry):
                ...
                (actor_loss, actor_loss_dict), actor_params, actor_opt_state = actor_update_fn(
                    actor_opt_state, agent_state.params.actor_params, agent_state, sample_batch, actor_key
                )
                agent_state = agent_state.replace(
            ...
            def _skip_actor(carry):
                agent_state, actor_opt_state = carry
                return agent_state, actor_opt_state, jnp.array(0.0), PyTreeDict(actor_loss=jnp.array(0.0))
            ...
            return (
                ...
                (enc_loss, critic_loss, actor_loss, enc_loss_dict, critic_loss_dict, actor_loss_dict),
            )

        # Retrieve global training steps from state metrics iterations
        # We do 1 gradient update per collected transition in theory
        num_updates_per_iter = self.config.rollout_episodes * self.config.env.max_episode_steps
        ...
        (
            ...
            (learn_key, agent_state, state.opt_state, replay_buffer_state, global_steps),
            iters,
            length=num_updates_per_iter,
        )

        # Episodic Checkpointing evaluate & replace
        perf = jnp.mean(eval_metrics.episode_returns) if self.config.checkpoint_metric == "mean" else jnp.min(eval_metrics.episode_returns)
        ...
        train_metrics = TD7TrainMetric(
            ...
            critic_loss=critic_loss,
            raw_loss_dict=PyTreeDict({**enc_loss_dict, **critic_loss_dict, **actor_loss_dict}),
