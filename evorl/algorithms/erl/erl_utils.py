...
from ..td3 import TD3TrainMetric, TD3NetworkParams
from .erl_base import ERLWorkflowBase, ERLTrainMetric
from .cemrl_base import CEMRLWorkflowBase
        ...
        if self.config.warmup_iters > 0:
            ...
            for i in range(num_fold_iters):
                state, train_metrics = scan_and_last(
                    _warmup_step, state, (), length=self.config.warmup_iters
                )
