...
def mlp_crossover(key: chex.PRNGKey, x1: chex.ArrayTree, x2: chex.ArrayTree, *,
    ...
    for i, ((path, param1), param2) in enumerate(zip(leaves1, leaves2)):
        if param1.ndim == 2:  # kernel
            key, ind_key, choice_key = jax.random.split(
                key, num=3)
            ...
            ind = jax.random.randint(
                ind_key, (num_crossover,), 0, param1.shape[0])
...
class MLPCrossover:
    def __init__(self, num_crossover_frac: float = 1.0):
        ...
        self.crossover_fn = jax.vmap(
            partial(mlp_crossover,
                    num_crossover_frac=num_crossover_frac),
            in_axes=(None, 0, 0)
        )
