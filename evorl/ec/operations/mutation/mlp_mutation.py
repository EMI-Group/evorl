...
def mlp_mutate(key: chex.PRNGKey, x: chex.ArrayTree, *,
    ...
    for i, (path, param) in enumerate(leaves):
        if param.ndim == 2:  # kernel
            ...
            (key, ind1_key, ind2_key, prob_key,
             normal_update_key, super_update_key,
             reset_update_key, ssne_prob_key) = jax.random.split(key, 8)

            ind_dim1 = jax.random.randint(
                ind1_key, (num_mutations,), 0, param.shape[0])
            ind_dim2 = jax.random.randint(
                ind2_key, (num_mutations,), 0, param.shape[1])

            prob = jax.random.uniform(prob_key, (num_mutations,))
            super_mask = prob < super_mut_prob
            reset_mask = jnp.logical_and(
                prob >= super_mut_prob, prob < reset_prob)
            # normal_mask = jnp.logical_not(jnp.logical_or(super_mask, reset_mask))

            updates = jax.random.uniform(
                normal_update_key, (num_mutations,)) * mut_strength * jnp.abs(param[ind_dim1, ind_dim2])
            super_updates = jax.random.normal(
                super_update_key, (num_mutations,)) * super_mut_strength * jnp.abs(param[ind_dim1, ind_dim2])
            reset_updates = jax.random.normal(
                reset_update_key, (num_mutations,))

            updates = jnp.where(super_mask, super_updates, updates)
            updates = jnp.where(reset_mask, reset_updates, updates)

            updates = jnp.clip(
                updates, -weight_max_magnitude, weight_max_magnitude)
            ...
            if vec_relative_prob > 0:
                ...
                (key, ind1_key, prob_key,
                 normal_update_key, super_update_key, reset_update_key,
                 ssne_prob_key) = jax.random.split(key, 7)
                ...
                updates = jax.random.uniform(
                    normal_update_key, (num_mutations,)) * mut_strength * jnp.abs(param[ind_dim1])
                super_updates = jax.random.normal(
                    super_update_key, (num_mutations,)) * super_mut_strength * jnp.abs(param[ind_dim1])
                reset_updates = jax.random.normal(
                    reset_update_key, (num_mutations,))
                ...
                ssne_prob = jax.random.uniform(ssne_prob_key)
                updates = jnp.where(
                    ssne_prob < ssne_probs[i]*vec_relative_prob, updates, jnp.zeros_like(updates))
