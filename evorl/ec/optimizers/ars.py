...
class ARS(EvoOptimizer):
    ...
    def tell(self, state: ARSState, xs: Params, fitnesses: chex.Array) -> ARSState:
        ...
        fit_1 = fitnesses[:half_pop_size]
        fit_2 = fitnesses[half_pop_size:]
        elite_idx = jax.lax.top_k(jnp.maximum(fit_1, fit_2), self.num_elites)[1]

        fitnesses_elite = jnp.concatenate([fit_1[elite_idx], fit_2[elite_idx]])
        # Add small constant to ensure non-zero division stability
        fitness_std = jnp.std(fitnesses_elite) + self.fitness_std_eps

        fit_diff = (fit_1[elite_idx] - fit_2[elite_idx]) / fitness_std

        grad = jtu.tree_map(
            # Note: we need additional "-1.0" since we are maximizing the fitness
            lambda z: (-weight_sum(z[elite_idx], fit_diff) / (self.num_elites)),
            noise,
        )
