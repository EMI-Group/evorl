...
class DDPGAgent(Agent):
    ...
    def init(self, key: chex.PRNGKey) -> AgentState:
        ...
        params_state = DDPGNetworkParams(q_params=q_params, 
    ...
    def compute_actions(self, agent_state: AgentState, sample_batch: SampleBatch, key: chex.PRNGKey) -> Tuple[Action, PolicyExtraInfo]:
        ...
        # add random noise
        noise_stddev = (self.action_space.high - self.action_space.low) * self.exploration_epsilon
    ...
    def loss(self, agent_state: AgentState, sample_batch: SampleBatch, key: chex.PRNGKey) -> LossDict:
        ...
        # ======= critic =======
        actor = self.actor_network.apply(
              agent_state.params.target_actor_params, obs)
        
        next_qs = self.q_network.apply(
            agent_state.params.target_q_params, next_obs, actor).squeeze(-1)
        
        target_qs = sample_batch.rewards + self.discount * (1 - sample_batch.dones) * next_qs
        
        qs = self.q_network.apply(
            agent_state.params.q_params, obs, actions).squeeze(-1)
        ...
        # [T*B, A]
        gen_actions = self.actor_network.apply(
            agent_state.params.actor_params, obs)

        actor_loss = - jnp.mean(self.q_network.apply(
            agent_state.params.q_params, obs, gen_actions).squeeze(-1))
    ...
    def compute_values(self, agent_state: AgentState, sample_batch: SampleBatch) -> chex.Array:
        ...
        return self.q_network.apply(
            agent_state.params.value_params, obs, actions).squeeze(-1)
     
    def compute_random_actions(self, key: chex.PRNGKey, sample_batch: SampleBatch) -> Tuple[Action, PolicyExtraInfo]:
        """
            get random actions for exploration used before "learning_starts" condition is met.
        """
        action = jax.random.uniform(key, shape=(sample_batch.obs.shape[0],)+self.action_space.shape, minval=self.action_space.low, maxval=self.action_space.high)
...
class DDPGWorkflow(OffPolicyRLWorkflow):
    ...
    def _build_from_config(cls, config: DictConfig):
        ...
        assert isinstance(env.action_space, Box), "Only continue action space is supported."
    ...
    def step(self, state: State) -> Tuple[TrainMetric, State]:
        ...
        def normal_step(state: State):
            ...
            def loss_fn(agent_state, sample_batch, key):
                ...
                loss = loss_weights['actor_loss'] * loss_dict['actor_loss'] * (state.metrics.iterations % self.config.actor_update_interval)
                loss += loss_weights['critic_loss'] * loss_dict['critic_loss']  
            ...
            target_q_params = soft_target_update(agent_state.params.target_q_params, agent_state.params.q_params, self.config.tau)
            target_actor_params = soft_target_update(agent_state.params.target_actor_params, agent_state.params.actor_params, self.config.tau)
